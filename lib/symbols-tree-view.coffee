...
console.log("symbols-tree-view.coffe")
module.exports =
  class SymbolsTreeView extends View
    ...
    initialize: ->
      ...
      @autoHideTypes = atom.config.get('language-baan.zAutoHideTypes')
      ...
      atom.config.observe 'language-baan.scrollAnimation', (enabled) =>
        @animationDuration = if enabled then 300 else 0

      @minimalWidth = 5
      @originalWidth = atom.config.get('language-baan.defaultWidth')
      atom.config.observe 'language-baan.autoHide', (autoHide) =>
        unless autoHide
          @width(@originalWidth)
        else
          @width(@minimalWidth)
    ...
    updateContextMenu: (types) ->
      ...
      if @cachedStatus[editor]
        ...
        @sortByNameScopes = atom.config.get('language-baan.sortByNameScopes')
    ...
    attached: ->
      ...
      @onChangeAutoHide = atom.config.observe 'language-baan.autoHide', (autoHide) =>
        unless autoHide
    ...
    resizeToFitContent: ->
      ...
      @width(10) # Shrink to measure the minimum width of list
